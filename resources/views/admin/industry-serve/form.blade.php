                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="card-title">
                                            <h3 class="text-center title-2">@isset($industryServe) Update @else Add New @endisset Industry Serve</h3>
                                        </div>
                                        <hr>
                                        <form action="@isset($industryServe){{ route('admin.industry-serve.update', $industryServe->id) }}@else{{ route('admin.industry-serve.store') }}@endisset" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <div class="row">
                                                <div class="col-6">
                                                    <div class="form-group">
                                                        <label for="title" class="control-label mb-1">Title</label>
                                                        <input id="title" name="title" type="text" class="form-control @error('title') is-invalid @enderror" value="{{ $industryServe->title ?? old('title') }}" required>
                                                        @error('title')
                                                ...
                                                <div class="col-6">
                                                    <div class="form-group">
                                                      @isset($industryServe)
                                                      <img id="icon" src="{{ asset('assets/images/uploads/industry-serve/' . $industryServe->icon) ?? asset('assets/images/defaultimage.png') }}" height="50px" width="50px" alt="your image" />
                                                      @endisset
