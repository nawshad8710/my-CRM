        ...
        <div class="main-content">
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-md-12">
                                <!-- DATA TABLE -->
                                <h3 class="title-5 m-b-35">Customer Query List</h3>
                                <div class="table-data__tool">
                                    ...
                                    <div class="table-data__tool-right">
                                        ...
                                        <a href="{{ route('admin.sales.query.add') }}" class="au-btn au-btn-icon au-btn--blue au-btn--small">
                                            <i class="zmdi zmdi-plus"></i>add query</a>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-data2">
                                        ...
                                        <tbody>
                                            @if(count($queries) > 0)
                                                ...
                                                <tr class="tr-shadow">
                                                    <td>{{ optional($query)->name }}</td>
                                                    <td>{{ optional($query)->phone }}</td>

                                                    <td>{{ optional($query)->email }}</td>
                                                    {{-- <td>{{ optional($sale)->name }}</td>
                                                    <td>{{ optional($sale)->email }}</td>
                                                    <td>{{ optional($sale)->phone }}</td> --}}
                                                    <td>{{ optional($query)->address }}</td>
                                                    <td>{{ optional($query)->company_name }}</td>
                                                    <td class="desc">
                                                        <?php 
                                                            $description =  strip_tags(html_entity_decode($query->description));
                                                        ...
                                                        @if (strlen($description) > 30)
                                                            {{ $desc }}
                                                            <a href="#" class="desc-text" onclick="descModalShow({{ $query->id }})"> <u>View Details</u></a>
                                                        @else
                                                            {{ $query->description }}
                                                        @endif
                                                        <!-- {!! Str::limit($description, $limit = 30, $end = '. . .<a href="#" class="desc-text" onclick="descModalShow()">View Details</a>') !!} -->
                                                        <div id="description{{ $query->id }}" class="d-none">
                                                            {!! $query->description !!}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        @if ($query->is_quotation_requested == 1)
                                                            Yes
                                                        @else
                                                            No
                                                        @endif
                                                    ...
                                                    <td>
                                                        @if ($query->is_proposal_requested == 1)
                                                            Yes
                                                        @else
                                                            No
                                                        @endif
                                                    ...
                                                    <td>
                                                        <div class="table-data-feature">
                                                            @if(has_access('products'))
                                                            <a href="{{ route('admin.sales.query.edit', $query->id) }}" class="item" data-toggle="tooltip" data-placement="top" title="Edit">
                                                                <i class="zmdi zmdi-edit"></i>
                                                            </a>
                                                            @endif
                                                            @if(has_access('products'))
                                                            <button class="item" data-toggle="tooltip" data-placement="top" title="Delete" onclick="deleteModalShow({{ $query->id }})">
			...
			<div class="modal fade" id="scrollmodal" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true">
				<div class="modal-dialog modal-lg" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="scrollmodalLabel">Query Description</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
